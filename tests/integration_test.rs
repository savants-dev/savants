...
fn test_mcp_unknown_tool_returns_cloud_message() {
    ...
    assert!(text.contains("requires savants.cloud"),
        "Expected cloud upgrade message, got: {}", text);
}
...
fn test_reindex_creates_caches() {
    ...
    let response = mcp_call(&msg);
    let text = parse_result_text(&response);
    assert!(text.contains("Indexed"), "Expected 'Indexed', got: {}", text);
    assert!(text.contains("entities"), "Expected entity count in: {}", text);

    // Check cache files exist
    let home = dirs::home_dir().unwrap();
    let repo_name = std::path::Path::new(repo_path).file_name().unwrap().to_str().unwrap();
    assert!(home.join(".savants/embeddings").join(format!("{}.bin", repo_name)).exists(),
        "Embedding cache not created");
    assert!(home.join(".savants/calls").join(format!("{}.json", repo_name)).exists(),
...
fn test_reindex_invalid_path() {
    ...
    let response = mcp_call(&msg);
    let text = parse_result_text(&response);
    assert!(text.contains("Not a directory"), "Expected error, got: {}", text);
}
...
fn test_semantic_search_no_index() {
    let msg = format!("{}\n{}\n", init_msg(),
        tool_call(2, "semantic_search", r#"{"query":"test","repo":"nonexistent_repo_xyz"}"#));
    let response = mcp_call(&msg);
    let text = parse_result_text(&response);
    assert!(text.contains("No index") || text.contains("Run"),
        "Expected no-index message, got: {}", text);
}

#[test]
fn test_file_skeleton_no_index() {
    let msg = format!("{}\n{}\n", init_msg(),
        tool_call(2, "file_skeleton", r#"{"file":"src/main.rs","repo":"nonexistent_repo_xyz"}"#));
    let response = mcp_call(&msg);
    let text = parse_result_text(&response);
    assert!(text.contains("No index") || text.contains("Run"),
        "Expected no-index message, got: {}", text);
...
fn test_callers_no_index() {
    let msg = format!("{}\n{}\n", init_msg(),
        tool_call(2, "callers", r#"{"function":"test","repo":"nonexistent_repo_xyz"}"#));
    let response = mcp_call(&msg);
    let text = parse_result_text(&response);
    assert!(text.contains("No index") || text.contains("Run"),
        "Expected no-index message, got: {}", text);
...
fn test_parser_extracts_rust_functions() {
    ...
    let repo_name = std::path::Path::new(repo_path).file_name().unwrap().to_str().unwrap();
    let msg = format!("{}\n{}\n", init_msg(),
        tool_call(2, "semantic_search",
            &format!(r#"{{"query":"cloud proxy server","repo":"{}","limit":3}}"#, repo_name)));
    let response = mcp_call(&msg);
    let text = parse_result_text(&response);
    assert!(text.contains("Semantic search") || text.contains("results"),
        "Expected search results, got: {}", text);
}
...
fn test_embedding_store_roundtrip() {
    ...
    let store_path = dirs::home_dir().unwrap().join(".savants/embeddings/test_roundtrip.bin");
...
fn test_cli_help() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("context engine"), "Help should mention context engine");
    assert!(stdout.contains("serve"), "Help should list serve command");
    assert!(stdout.contains("reindex"), "Help should list reindex command");
}
...
fn test_cli_version() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("savants"), "Version should contain 'savants'");
}
...
fn test_cli_status() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Status") || stdout.contains("Cloud") || stdout.contains("Search"),
